...
begin
  ...
  HideCaret(TEditX(FPParentEdit).Handle);
  // Cursor:=crUpDown;
  FDown := true;
...
begin
  ...
  FDown := false;
  ShowCaret(TEditX(FPParentEdit).Handle);
  if FPParentEdit <> nil then
...
procedure TEditX.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  inherited;
  ShowCaret(Handle);
